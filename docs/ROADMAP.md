...
*   **Tool Isolation:** Orchestrator filters tools by agent's assigned skills; unauthorized tool execution rejected.
*   **Gateway-backed `model_route` integration:** Agents default to `gateway` provider, model resolution via route.
*   **Multi-Agent Orchestration:** Sequential + Fan-out + Cross-Harness Review patterns.
*   **Setup Checklist alignment** with `docs/plans/ui-main-flow.md` Phase 8 spec.
...
*   Remote coding sessions through Discord/Telegram/Slack (§5.11) — deferred until core features stabilize.
...
## 9. new update:
- agent không cần gán với skill trực tiếp. là chỉ gán vs role và level group model để  làm các task vs mức độ phức tạp tương ứng hoặc là auto chọn model theo mức độ phức tạp của task.

- Agent nạp skill bằng cách nào?
- 
- Step 1: khi 1 task được con người tạo ra 
- Step 2: Agent có thể phân tích chia nhỏ thành các task con
sư dung promt của openspec để tạo theo format đó. bao gồm cả Workflow Progress cho task đó:
Ví dụ task đó không có UI thì không cần FE join vào trong flow....
- Step 3: Con người review task con và spec xem hợp lý chưa
- Step 4: Agent thực hiện theo Workflow Progress....
- Step 5: Con người review và phê duyệt code cuối cùng.

## 9. New Updates (Role-Based Skills & Centralized Repositories)

- **Decoupled Agent-Skill Model**: Agents are no longer statically bound to specific skills in the database. Instead, they are defined by their **Role** and **Autonomy Level** (Model Group). The system dynamically evaluates task complexity and automatically selects the appropriate model or routes roles.
- **Dynamic Skill Loading**: Agents dynamically discover and load skills from the filesystem during the analysis phase of a task. The orchestrator automatically retrieves tool definitions and playbooks dynamically based on the active role and task requirements.
- **Centralized Skills Directory**: A centralized directory is established at `skills/` parallel to the workspace (`auto_code_os/`) to organize skills cleanly:
  - `skills/system/`: Stores system-wide, global agent skills of the system.
  - `skills/workspace/`: Stores workspace-specific/project-specific agent skills.
- **Dynamic Execution Isolation**: During execution, the orchestrator mounts `/skills` or reads from the centralized path using secure APIs (e.g., `read_skill` tool) to execute domain-specific tasks without leaking host paths.