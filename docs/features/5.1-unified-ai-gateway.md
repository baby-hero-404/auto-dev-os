...
**Acceptance criteria:** Admin can configure multiple provider credentials with model list per Model Level Group (Fast/Balanced/Powerful), route requests by level or combo route, and observe usage/cooldown/audit events.

**Mục tiêu:** Cung cấp một lớp gateway tập trung quản lý **định tuyến model, budget tracking, format translation và fallback tự động** — thay thế cách tiếp cận đơn giản "lưu API key + chọn model" trước đây. Agent và client chỉ gọi vào gateway với một route ổn định, không cần biết provider thật phía sau.

> **Tiến độ triển khai (Đang thực hiện):**
> - ✅ **Hoàn thành Backend Core:** Đã implement Budget tracking (atomic), Multi-key pool rotation (fill-first/round-robin), Cooldown recovery.
...
> - ⏳ **Còn lại (Remaining):** Cấu hình UX cho danh sách model theo Model Level Group, và mở rộng độ phủ của Telemetry/Audit Log.
