...
```
Task nhận vào → Phân tích độ phức tạp (Easy / Medium / Hard)
  ├── Easy    → Sequential (1 agent)
  ├── Medium  → Fan-out steps (Backend + Frontend) + Reviewer Validation (1 reviewer agent)
  └── Hard    → Sử dụng cấu trúc tương tự Medium workflow (Fan-out steps) + Reviewer Validation
→ Mỗi Agent chạy trong Sandbox cách ly
→ Kết quả merge → Code Review → PR tạo tự động
```
