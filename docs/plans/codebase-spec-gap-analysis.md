...
The implementation already has the core workflow skeleton: task analysis, spec review pause, coding, review, fix, test, PR creation, checkpoint recovery, and a backend `max_review_fix_cycles` limit. The main gaps are around newer policy requirements that were added to the specs:

- Context loading is described as Step 0 but is not represented as a workflow step or task state.
- Easy tasks can still be auto-approved without checking high-risk domains.
- PR creation skips the specified `pr_ready` task state and moves directly to `human_review`.
- Parallel FE/BE branch isolation is described in the spec but not implemented as real branch/worktree behavior.
- Project review policy settings are stored, but the workflow does not consistently enforce them.
- Review-loop limits exist in backend logic, but the UI and PR metadata do not expose the limit or warning clearly.

## Gap Matrix

| Area | Spec Requirement | Current Implementation | Severity | Primary Files |
| :--- | :--- | :--- | :--- | :--- |
| Context loading | Workflow starts with Step 0: load repository structure, branch conventions, CI config, test commands, and architecture docs before analysis. | Missing as a workflow step. Prompt assembly and context retrieval exist, but execution starts at `analyze` and task status moves to `analyzing`. | High | `server/internal/workflow/step.go`, `server/internal/orchestrator/orchestrator_worker.go`, `server/internal/orchestrator/orchestrator_steps.go`, `web/src/app/projects/[id]/tasks/[taskID]/monitor/page.tsx` |
| Risk-aware auto-approval | Easy tasks that touch high-risk domains must pause for human spec review. | Missing. `StepAnalyze` auto-approves easy tasks when autonomy/policy allows, without checking `risk_domains` or high-risk file patterns. | High | `server/internal/orchestrator/orchestrator_steps.go`, `server/pkg/models/task.go` |
| Task completion policy | PR creation should set `pr_ready`; final review then moves to `human_review`; only merge completes the task. | Partial/missing. PR output says `pr_ready_for_human_approval`, but persisted task status is `human_review`. `TaskStatusPrReady` does not exist. | Medium | `server/pkg/models/task.go`, `server/internal/orchestrator/orchestrator_steps.go`, task detail/monitor UI |
| Parallel FE/BE branch isolation | Backend and frontend agents use separate branches/worktrees, then merge into an integration branch. | Mostly missing. Workflow has `code_backend`, `code_frontend`, and `merge` steps, but PR creation uses one hardcoded `autocode/task-{task_id}` branch. No durable per-agent branch metadata is tracked. | Medium | `server/internal/workflow/step.go`, `server/internal/orchestrator/orchestrator_steps.go`, `server/internal/orchestrator/orchestrator_workspace.go` |
| Review policy enforcement | Project review policy should control spec review, PR review, auto-merge, and reviewer requirements. | Partial/inconsistent. `AutoReviewPolicy` is used by `TaskService.Analyze`, but orchestrator `StepAnalyze` mostly uses agent autonomy and complexity. PR approve immediately merges; reviewer-count and test/conflict gates are not enforced. | Medium | `server/internal/service/task.go`, `server/internal/orchestrator/orchestrator_steps.go`, `server/internal/handler/pr.go`, `server/internal/orchestrator/orchestrator.go`, `web/src/components/projects/project-profile.tsx` |
| Bounded review-fix loop visibility | Enforce `max_review_fix_cycles`; when exceeded, mark metadata and warn reviewers in PR/UI. | Partial. Backend reads `Project.MaxReviewFixCycles` and skips fixing when the limit is reached. Frontend settings do not expose `max_review_fix_cycles`; PR summaries do not include a `review_limit_exceeded` warning. | Low | `server/internal/orchestrator/orchestrator_steps.go`, `server/internal/orchestrator/pr_generator.go`, `server/pkg/models/project.go`, `web/src/components/projects/project-profile.tsx`, `web/src/lib/types.ts` |
| Test strategy fidelity | Specs call for targeted tests after code/fix and full relevant tests before PR. | Partial. `StepTest` runs one generic command (`go test ./...` or `npm test`). There is no separate targeted test step after coding/fixing. | Low | `server/internal/orchestrator/orchestrator_steps.go`, `server/internal/workflow/step.go` |
| Conflict escalation | Parallel merge conflicts should be auto-resolved based on ownership or paused for human resolution. | Missing. `StepMerge` captures a diff and marks changes reconciled; it does not perform branch merges, detect conflicts, persist conflict state, or pause for human resolution. | Medium | `server/internal/orchestrator/orchestrator_steps.go`, `server/internal/workflow/step.go`, workflow UI |
