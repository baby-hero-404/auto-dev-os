...
func TestPRGenerator_GenerateSummary(t *testing.T) {
	...
	summary := gen.GenerateSummary(context.Background(), task, nil, files, "", nil, nil, false)
...
`
	...
	summary := gen.GenerateSummary(context.Background(), task, agent, []string{"auth.go"}, diffText, testResult, nil, false)
