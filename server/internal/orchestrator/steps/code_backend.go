...
func (s *CodeBackendStep) Execute(ctx context.Context, stepCtx workflow.StepContext) (StepResult, error) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		if attempt >= 2 && s.worktree != nil {
			...
			if errReset := s.worktree.ResetRoleWorktrees(ctx, s.rt.Task, backendAgent, worktreeSuffix); errReset != nil {
		...
		out, err = s.llm.RunLLMStep(llmCtx, s.rt.Task, backendAgent, s.rt.JobID, stepCtx.StepID, currentInstruction)
		if err != nil {
			return nil, err
		}
