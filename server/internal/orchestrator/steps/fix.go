...
func (s *FixStep) Execute(ctx context.Context, stepCtx workflow.StepContext) (StepResult, error) {
	...
	if s.llm != nil {
		...
		instruction += "IMPORTANT: For the patch text, you MUST generate a valid Unified Diff. Ensure that your hunk headers (@@) have the exact correct line counts matching the target file. Your diff paths MUST include the repository name prefix (e.g., --- a/repo-name/filepath).\n\n"
