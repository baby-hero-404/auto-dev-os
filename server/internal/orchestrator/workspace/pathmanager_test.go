...
func TestRepoRelativeToWorkspace(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := RepoRelativeToWorkspace(tt.repo, tt.path)
			if got != tt.expected {
				t.Errorf("RepoRelativeToWorkspace(%q, %q) = %q, want %q", tt.repo, tt.path, got, tt.expected)
			}
		})
	}
}
