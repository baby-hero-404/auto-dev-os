...
func TestFindRepoWorkspaceByPath(t *testing.T) {
	...
	resolved, err := orchestrator.FindRepoWorkspaceByPath(context.Background(), task, pathInsideApiClient)
	if err != nil {
	...
	resolvedApi, err := orchestrator.FindRepoWorkspaceByPath(context.Background(), task, pathInsideApi)
	if err != nil {
