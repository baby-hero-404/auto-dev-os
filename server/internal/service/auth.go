...
func (s *AuthService) Login(ctx context.Context, input models.LoginInput) (*models.AuthResponse, error) {
	...
	if err != nil {
		return nil, ErrValidation("invalid email or password")
	}
...
func (s *AuthService) VerifyToken(token, expectedType string) (*TokenClaims, error) {
	...
	if len(parts) != 3 {
		return nil, ErrValidation("invalid token")
	}
	signed := parts[0] + "." + parts[1]
	want := signJWTPart(signed, s.jwtSecret)
	if !hmac.Equal([]byte(want), []byte(parts[2])) {
		return nil, ErrValidation("invalid token signature")
	}
	payload, err := base64.RawURLEncoding.DecodeString(parts[1])
	if err != nil {
		return nil, ErrValidation("invalid token payload")
	}
	var claims TokenClaims
	if err := json.Unmarshal(payload, &claims); err != nil {
		return nil, ErrValidation("invalid token claims")
	}
	if claims.Type != expectedType {
		return nil, ErrValidation("invalid token type")
	}
	if time.Now().Unix() >= claims.Expires {
		return nil, ErrValidation("token expired")
	}
	return &claims, nil
}
